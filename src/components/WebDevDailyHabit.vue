<template>
  ...
  <p class="text-center my-5">
    Inspired from
    <a href="https://theicedev.netlify.app/" target="_blank" rel="noreferrer" class="underline font-bold">
      @dev.iceice
    </a>
  </p>
  <p class="text-center my-5">
    Note: Some days are kept under 1 hour for light studying to avoid burnout.
  </p>
  <div>
    <div class="tracker-info card p-5">
      ...
      <div class="flex flex-col gap-2 justify-center items-center mt-3">
        <div class="flex gap-1.5">
          ...
          <h3 class="font-bold font-xl">
            Total Time: {{ displayTimeFormatted }}
          </h3>
        </div>
        <div class="flex gap-1.5">
          <svg class="w-[20px]" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="data-info-wrapper flex gap-5 justify-start flex-wrap">
      <div class="flex flex-col gap-3 items-center p-5 card" v-for="data in webDailyTracker" :key="data.day">
        <div class="tooltip" @click="toggleModal(data)">
          i
        </div>
        <div>
        ...
        <div>
          <h3 class="font-bold text-4xl">
            {{ data.time_consumed.hour }} :
            {{ addZeros(data.time_consumed.minutes) }}
          </h3>
        ...
        <div class="">
          ...
          <div class="flex gap-1.5 justify-center">
            <img v-for="techUsed in data.tech_used" :src="getImageBadge(techUsed)" :title="techUsed + ' logo'"
              :alt="techUsed + ' logo'" :class="techUsed + '-logo'" width="20" height="20" />
          </div>
        </div>
      </div>
    </div>
  ...
  <div v-if="showModalProps">
    <ModalProps :header="selectedItem?.day" :date="selectedItem?.date" :text="selectedItem?.journal" :techUsed="selectedItem?.tech_used" @close="showModalProps = false" />
  </div>
</template>
...
import ModalProps from "./ModalProps.vue";

const { webDailyTracker, trackerError, loadWebDailyTracker } =
  getWebDailyTracker();
const { skills, skillError, loadSkills } = getSkills();

const showModalProps = ref(false);
...
const getTimeData = (time) => {
  return webDailyTracker.value
    ? webDailyTracker.value.reduce((total, item) => {
...
const displayTimeFormatted = computed(() => {
  let totalConvertedMinutes =
    totalTimeHours.value * 60 + totalTimeMinutes.value;
  let totalConvertedHours = Math.floor(totalConvertedMinutes / 60);
  let totalConvertedMinutesOnly = Math.round(
    (Math.round((totalTimeHours.value + totalTimeMinutes.value / 60) * 100) /
      100 -
      totalConvertedHours) *
    60
  );
...
const filteredSkills = computed(() => {
  return skills.value.filter((data) => data.category !== "multimedia");
});
...
function toggleModal(data) {
  ...
  showModalProps.value = !showModalProps.value;
  console.log( showModalProps.value)
}
